...
public class CBKGridManager : MonoBehaviour {
	...
	public static CBKGridManager instance;
	
	[SerializeField]
	List<int> roadLines;
	
	List<CBKWalkableSpace> walkableSpaces = new List<CBKWalkableSpace>();
	...
	public CBKGridNode randomWalkable{
		get
		{
	...
	{
		return new CBKGridNode(Mathf.Clamp((int)(point.x / spaceSize), 0, gridSize-1), Mathf.Clamp((int)(point.z / spaceSize), 0, gridSize-1));
	}
	...
	/// Screen position.
	/// </param>
	public CBKGridNode ScreenToPoint(Vector3 screenPos)
	{
	...
	public bool CanMoveToNeighbor(CBKGridNode start, CBKGridNode end)
	{
		//Get dx and dy
