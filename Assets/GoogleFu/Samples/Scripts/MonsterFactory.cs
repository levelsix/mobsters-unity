...
namespace GoogleFuSample
{
	...
	{
		public Monster _monsterPrefab;	// Prefab that represents a Monster
		public Item _itemPrefab;		// Prefab that represents an Item
		CharacterStats _statsDb;		// Example of an Object based Database.
		Items _itemsDb;					// Example of a Static Database.
		...
		void Start ()
		{
			...
			if ( statsdbobj != null )
			{
				...
				_statsDb = statsdbobj.GetComponent<CharacterStats>();
			}
		...
		void OnGUI ()
		{
			...
			{
				Spawn((int)CharacterStats.rowIds.AI_GOBLIN);
			}
			
			if(GUI.Button(new Rect(20,70,90,20), "Troll")) 
			{
				Spawn((int)CharacterStats.rowIds.AI_TROLL);
			}
		...
		void Spawn (int indexToSpawn)
		{
			if ( _monsterPrefab != null && _statsDb != null )
			{
				...
				Monster myMonster = (Monster)Instantiate( _monsterPrefab );
				...
				if ( _itemPrefab != null && _itemsDb != null )
				{
					...
					Item myItem = (Item)Instantiate( _itemPrefab );
