...
static public class NGUIText
{
	...
	static public void Print (string text, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols, float symbolScale = 1f)
	{
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (symbol != null)
			{
				...
				if (Mathf.RoundToInt(x + symbol.advance * fontScale * symbolScale) > regionWidth)
				{
					if (x == 0f) return;
